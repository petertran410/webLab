        ...
        <%
            if (cUsername == null || cPass == null) {
                out.println("<h1 style=\"color:#8b2f8a; text-align: center;\">Edusoft Web HCMIU Edu VN</h1>"
                        ...
                        + "        <tr>\n"
                        + "            <td>Student's ID</td>\n"
                        + "            <td><input class=\"form-control\" type=\"text\" name=\"username\" placeholder=\"ITITIU20264\" required></td>\n"
                        + "        </tr>\n"
                        + "        <tr>\n"
                        + "            <td>Password</td>\n"
                ...
                if (inputName != null && inputName.trim().length() > 0) {
                    if (inputPass != null && inputPass.trim().length() > 0) {
                        try {
                            ...
                            connection = DriverManager.getConnection(connectionURL, "Admin", "admin"); 
                            statement = connection.createStatement();
                            rs = statement.executeQuery("SELECT * FROM student where StudentID = \"" + inputName + "\" "); 
                            if (rs.next()) {
